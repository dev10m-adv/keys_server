...
async function _verify(req, res, next) {
  ...
  if (!VALID_ALGORITHMS.has(sigAlgorithm)) {
    return res.status(401).json({ error: 'unsupported_algorithm', message: `Unknown sigAlgorithm: ${sigAlgorithm}` });
  }
  ...
  if (Math.abs(Date.now() - timestamp) > TIMESTAMP_WINDOW_MS) {
    return res.status(401).json({ error: 'request_expired', message: 'Request timestamp is outside the 30-second window' });
  }

  // 4. JTI replay check
  if (hasJti(jti)) {
    return res.status(401).json({ error: 'replayed_request', message: 'This request JTI has already been used' });
  }

  // 5. Body hash check — prevents tampering with the body after signing
  const rawBody = req.rawBody || Buffer.alloc(0);
  if (sha256Hex(rawBody) !== bodyHash) {
    return res.status(401).json({ error: 'body_tampered', message: 'Request body SHA-256 does not match payload.bodyHash' });
  }

  // 6. OpenPGP preference enforcement — if the user has ANY active OpenPGP key,
  //    S/MIME-signed requests are rejected to prevent algorithm downgrade
  if (sigAlgorithm === 'smime') {
    ...
    if (hasOpenpgp) {
      return res.status(403).json({
        ...
        message: 'An active OpenPGP key exists for this account — only OpenPGP-signed requests are accepted',
      });
    }
  }
  ...
  try {
    ...
    const code = err.code === 'no_key_for_algorithm' ? 'invalid_signature' : 'invalid_signature';
    return res.status(403).json({ error: code, message: err.message });
  }

  // 8. Mark JTI as used — only after all checks pass so an invalid request
  //    cannot poison the cache and prevent legitimate retries
  addJti(jti);
